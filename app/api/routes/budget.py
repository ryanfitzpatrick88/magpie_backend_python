...
def create_budget_item(budget: BudgetCreate, db: Session = Depends(get_user_db), current_user: User = Depends(get_current_user)):
    return create_budget(db, budget)

@router.get("/", response_model=List[BudgetInDB])
def read_budgets(skip: int = 0, limit: int = 100, db: Session = Depends(get_user_db), current_user: User = Depends(get_current_user)):
    budgets = get_budgets(db, skip=skip, limit=limit)
    return budgets

@router.get("/{budget_id}", response_model=BudgetInDB)
def read_budget(budget_id: int, db: Session = Depends(get_user_db)):
...
def update_budget_item(budget_id: int, budget: BudgetUpdate, db: Session = Depends(get_user_db)):
...
def delete_budget_item(budget_id: int, db: Session = Depends(get_user_db)):
